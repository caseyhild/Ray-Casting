        ...
        switch (texName) {
            case "ball" -> {
                for (int i = 0; i < SIZE * SIZE; i++) {
                    int shade = 255 - (int) (255 * Math.sqrt(Math.pow((SIZE * (SIZE / 2.0 - 1) + SIZE / 2.0 - 1) % SIZE - i % SIZE, 2) + Math.pow((SIZE * (SIZE / 2.0 - 1) + SIZE / 2.0 - 1) / SIZE - (double) i / SIZE, 2)) / 20.0);
                    if (shade <= 1)
                        shade = 1;
                    if (Math.sqrt(Math.pow((SIZE * (SIZE / 2.0 - 1) + SIZE / 2.0 - 1) % SIZE - i % SIZE, 2) + Math.pow((SIZE * (SIZE / 2.0 - 1) + SIZE / 2.0 - 1) / SIZE - (double) i / SIZE, 2)) < 20)
                        pixels[i] = rgbNum(0, 0, shade);
                    else if (Math.sqrt(Math.pow((SIZE * SIZE / 2.0 + SIZE / 2.0) % SIZE - i % SIZE, 2) + Math.pow((SIZE * SIZE / 2.0 + SIZE / 2.0) / SIZE - (double) i / SIZE, 2)) < 20)
                        pixels[i] = rgbNum(0, 0, shade);
                    else
